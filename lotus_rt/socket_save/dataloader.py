...
# (shell2) ./test_socket.py sock_tmp 1017 send
        ...
        while True:
            ...
            if len(self._task_info[self._rcvd_idx]) == 2:
                ...
                if self.log_check:
                    ...
                    self._dataset.log_file.send(f'main ({self.pid}): SBatchConsumed_{batch_id}, {time.time_ns()}, 1000\n'.encode())
                    return _data
                else:
            ...
            if idx != self._rcvd_idx:
                ...
                if self.log_check:
                    ...
                    self._dataset.log_file.send(f'main ({self.pid}: SBatchWait_{idx}, {end_wait}, 1000\n'.encode())
                ...
                if self.log_check:
                    ...
                    _data = self._process_data(data)
                        ...
                        # 1 microsec below is just a placeholder 
                        #f.write(f'SBatchConsumed_{batch_id},{time.time_ns()},1000\n')
                    self._dataset.log_file.send(f'main ({self.pid}): SBatchWait_{batch_id}, {start_wait}, {end_wait-start_wait}\n'.encode())
                    self._dataset.log_file.send(f'main ({self.pid}): SBatchConsumed_{batch_id}, {time.time_ns()}, 1000\n'.encode())
                    return _data
                else:
                    return self._process_data(data)
    ...
    def _try_put_index(self):
        ...
        if self.log_check:
            ...
            self._dataset.log_file.send(f'main ({self.pid}): SBatchQueued_{self._send_idx}, {time.time_ns()}, 1000\n'.encode())
