...
parser.add_argument("--per-batch-train-time-log-file", type=str, required=True, help="Per batch elapsed time (in ms) of time taken to move data to GPU, forward and backward pass.")
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    if args.train_loop <= 5:
        print('--train-loop argumernt is too small. Please set it to a value greater than 5.')
        exit(1)
...
def train(train_loader, model, criterion, optimizer, epoch, device, args):
    ...
    for i, (images, target) in enumerate(train_loader):
        ...
        if i==4:
            ...
            open(args.per_batch_train_time_log_file,'w').write(f"{start_cuda.elapsed_time(end_cuda)} ms")
        if args.profile:
            train_p.step()
