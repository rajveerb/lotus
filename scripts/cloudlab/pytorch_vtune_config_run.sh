...
do
    ...
    do
        result_dir_=${mem_access_result_dir}b${batch_size}_gpu${num_gpu}
        vtune -collect memory-access -data-limit 0 -result-dir ${result_dir_} ${python_path} ${program_path} ${dataset_path} -b ${batch_size} -j ${num_gpu} --epochs ${num_epochs} --val-loop 0;
    done
done
...
#         vtune -collect memory-consumption -quiet -result-dir ${result_dir_} ${python_path} ${program_path} ${dataset_path} -b ${batch_size} -j ${num_gpu} --epochs ${num_epochs}
#     done
# done
...
#         vtune -collect hotspots -quiet -result-dir ${result_dir_} ${python_path} ${program_path} ${dataset_path} -b ${batch_size} -j ${num_gpu} --epochs ${num_epochs}
